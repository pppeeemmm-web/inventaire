...
| `/atelier` | Main team portal (tabbed UI); `?tab=inventory` / `?tab=sales` / `?tab=pipeline` / `?tab=production` / `?tab=stock-take` / `?tab=notes` / `?tab=reports` / `?tab=exhibitions` / `?tab=concepts` / `?tab=themes` / `?tab=logistics` / `?tab=vault` / `?tab=fiscal` / `?tab=broadcast` redirect to segment routes; see §3 |
...
| [`lib/atelier/tab-routes.ts`](../lib/atelier/tab-routes.ts) | **`atelierTabHref`**, legacy `?tab=inventory|sales|pipeline|production|stock-take|notes|reports|exhibitions|concepts|themes|logistics|vault|fiscal|broadcast` → segment route redirects |
