...
export async function generateWorksTablePdf(
  ...
  const d = dict[lang]
  const pdfDocTitle = d.report_title
  ...
  try {
    ...
    session.from('Contact').select('ContactID, NomInstitution, Nom, "Prénom", Ville, Pays'),
    ...
    const drawHeader = () => {
      ...
      for (let i = 0; i < headers.length; i++) {
        const h = String(headers[i] ?? '').slice(0, 48)
    ...
    doc.text(`${pdfDocTitle} — ${new Date().toLocaleDateString(locale)}`, left0, y, { width: pageW })
    y += 22
    ...
    console.error('[generateWorksTablePdf]', e)
    return { error: 'fail' }
  }
}
