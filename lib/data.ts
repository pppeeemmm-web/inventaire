...
export function imageUrl(file: string | null | undefined): string | null {
  ...
  // Prefer R2 if configured, fall back to Supabase Storage
  const base = R2 || STR
  return `${base}/${encodeURIComponent(file)}`
}

/**
 * Thumbnail URL — points directly at thumbs/<filename>.jpg on R2.
 * Generated by sharp at upload time (400px JPEG). No optimizer needed.
 * Falls back to full imageUrl if R2 is not configured.
 *
...
export function thumbUrl(
  ...
  if (!file) return null
  if (file.startsWith('http')) return file
  if (!R2) return imageUrl(file)
  // Strip extension, replace with .jpg — matches what uploadImage generates
  const base = file.replace(/\.[^.]+$/, '')
  return `${R2}/thumbs/${encodeURIComponent(base)}.jpg`
}
