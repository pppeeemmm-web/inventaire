...
> **ARCHIVED — programme complete (owner, 2026-05-25; on `main`).** Do not execute from this file. Live checklist: [`../TODO.md`](../TODO.md).

> **STATUS:** Slices 1–8 shipped (incl. 3B, Slice 4 i18n, Slice 7 Phase 2 graph CSV backup, Slice 1 PWA/offline, portal shell persistence).

> **Cursor Auto Mode plan, generated 2026-05-22.** Supersedes `universe V3.md` after a Claude Code audit of `origin/main` and integration of the Qdrant + Ollama addendum. Historical execution guide. **Owner GO required** before any new edits (`CLAUDE.md`).
...
- UX layer = Slice 3 (routes + QR Physical Bridge) + Slice 4 (i18n messages). Slice 1 PWA/offline shipped with V5 closure (2026-05-25).
...
| 10 | **1** | PWA/SW (Serwist) + R2 cache — **complete**                | — |
...
## Slice 7 — Docs hygiene & Analog Fallbacks — **complete**
...
- **Docs consolidation (2026-05-23):**
  - **Keep:** `CLAUDE.md`, `AGENTS.md`, `SITE_MAP.md`, `docs/README.md`, `docs/TODO.md`, `docs/SYSTEM_LEDGER.md`, `docs/BACKUP_RECOVERY.md`, `docs/CONSTELLATION.md`, `docs/BROADCAST.md`, `docs/BROADCAST_OUTSIDE_CHAIN.md`, slim `docs/PROJECT_SYNTHESIS.md`, `docs/feature-graph.md`.
  - **Archive:** `docs/archive/` — this plan, old hybrid plan, Phase 2 broadcast snapshot, 2026-05-15 `STATUS.md`, completed `HANDOFF_SLICE*.md` (Slices 1, 3–8).
...
## Slice 1 — PWA / Service Worker — **complete**
...
**Shipped:** `@serwist/next`, `app/sw.ts`, `app/~offline/page.tsx`, `lib/mobile/offline-work-queue.ts`, `AtelierSWRegistrar` on hub/atelier layouts. Phase 2 items (Background Sync, R2 `Cache-Control`, `revalidateTag` shell) remain optional polish — not blocking V5 closure.
...
| Broadcast tab (Slice 3) | `app/atelier/broadcast/page.tsx`, `app/atelier/broadcast/_components/Broadcast.tsx` |
...
## Post–V5 — Atelier shell across segment tabs — **shipped (2026-05-23)**

**Implemented:** [`app/atelier/(portal)/layout.tsx`](../app/atelier/(portal)/layout.tsx) loads the shell once with `shellPersistsAcrossTabs: true`; segment routes use [`portal-tab-page.tsx`](../app/atelier/(portal)/portal-tab-page.tsx) (`return null`). [`TeamPortalClient`](../components/atelier/TeamPortalClient.tsx) skips duplicate postPaint when the shell persists. [`loadAtelierShellProps`](../lib/atelier/load-atelier-shell-props.ts) uses stable `atelierShellNonce: 0` and tab-aware catalogue chunk loading.

**Optional polish (not V5):** slimmer cold-start loaders per tab (Audit/Logistics) — see [`../TODO.md`](../TODO.md) post-V5 section.
