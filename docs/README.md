# Documentation index



**Truth order:** live code → [`CLAUDE.md`](../CLAUDE.md) → [`SITE_MAP.md`](../SITE_MAP.md) → files below.



| Doc | Use when |

|-----|----------|

| [`TEAM_MEMBER_GUIDE.md`](./TEAM_MEMBER_GUIDE.md) | **Onboarding:** every Atelier tab + recent features for **non-admin** team members |

| [`TODO.md`](./TODO.md) | Version-controlled checklist (ops deadlines, dev blocks, optional follow-ups) |

| [`feature-graph.md`](./feature-graph.md) | Graph layer index (Slices 5–8 + export/backup pointers) |

| [`PROJECT_SYNTHESIS.md`](./PROJECT_SYNTHESIS.md) | Onboarding: stack boundaries, broadcast chain, where truth lives |

| [`SYSTEM_LEDGER.md`](./SYSTEM_LEDGER.md) | System tab UI contract (also loaded in-app for reference MD) |

| [`BACKUP_RECOVERY.md`](./BACKUP_RECOVERY.md) | Off-site DB backups + restore drill |

| [`CONSTELLATION.md`](./CONSTELLATION.md) | Constellation graph feature contract |

| [`BROADCAST.md`](./BROADCAST.md) | Broadcast HTTP API + DB columns + key files |

| [`BROADCAST_OUTSIDE_CHAIN.md`](./BROADCAST_OUTSIDE_CHAIN.md) | Make.com → Buffer → Slack (outside this repo) |



**Archive:** [`archive/`](./archive/) — **V5 programme plan**, slice handoffs, superseded plans (do not execute from there). Live checklist = [`TODO.md`](./TODO.md).



**i18n copy pipeline:** [`archive/HANDOFF_SLICE4.md`](./archive/HANDOFF_SLICE4.md) (archived handoff; rules also in `CLAUDE.md` + `AGENTS.md`).


