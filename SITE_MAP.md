...
| `/atelier` | Main team portal (tabbed UI); `?tab=inventory` / `?tab=sales` / `?tab=pipeline` / `?tab=production` / `?tab=stock-take` / `?tab=notes` / `?tab=reports` / `?tab=exhibitions` / `?tab=concepts` / `?tab=themes` / `?tab=logistics` / `?tab=vault` / `?tab=fiscal` / `?tab=broadcast` / `?tab=audit` / `?tab=constellation` redirect to segment routes; `?map=<uuid>` redirects to `/atelier/constellation?map=`; see §3 |
...
**Recent doc sync (2026-05-23):** Slice 3 segment routes `/atelier/inventory`, `/atelier/sales`, `/atelier/pipeline`, `/atelier/production`, `/atelier/stock-take`, `/atelier/notes`, `/atelier/reports`, `/atelier/exhibitions`, `/atelier/concepts`, `/atelier/themes`, `/atelier/logistics`, `/atelier/vault`, `/atelier/fiscal`, `/atelier/broadcast`, `/atelier/audit`, `/atelier/constellation` — **Slice 3 tab routes complete**; shared `loadAtelierShellProps` + `tab-routes`; legacy `?tab=` / `?map=` redirects for migrated tabs.
