...
- **Atelier sidebar (narrow)**: First group **Terrain / Field** — tabs `inventory` → `production` → `stock-take` → `map`, then existing buckets (`TeamPortalClient` `GROUPS` when `atelierNarrow`).
- **Rings (shorthand):** **A** — Atelier narrow chrome (subset batch, stacked headers). **B** — Hub field launcher (`HubLauncherClient` `FIELD_ROWS`) + mobile action bar / voice sheet. **B.3** — PWA Web Share Target + `share_inbox` + share triage (import form on iOS). **C** — Deep-linked field verb routes that still render [`FieldToolStubPage`](components/atelier/FieldToolStubPage.tsx) (`/atelier/session/new`, `/atelier/capture`, `/atelier/documents/new`, `/atelier/issue/new`, `/atelier/triage`) until replaced by real flows.
