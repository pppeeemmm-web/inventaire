    ...
    <div className="w-page-enter">
      ...
      <div className="w-viewport">
        {sequence.map((item, idx) => {
          ...
          const isLastWork = idx === lastWorkIdx && item.type === 'work'
          const rawOpacity = dist < 0
            ? Math.pow(Math.max(0, (dist + BIRTH_DIST) / BIRTH_DIST), 4)
            : Math.max(0, 1 - Math.max(0, dist - STEP * 0.3) / (STEP * 0.4))
          // Hold last painting fully visible while end overlay covers the screen
          const opacity = isLastWork && endOpacity > 0 ? Math.max(rawOpacity, endOpacity) : rawOpacity
          ...
          if (item.type === 'header') {
            ...
            return (
              <div key={`header-${idx}`} className="w-depth-item" style={{
                ...
                zIndex, pointerEvents: Math.abs(dist) < 2000 ? 'auto' : 'none',
      ...
      {/* End overlay — full-screen opaque paper, always mounted */}
      <div style={{
        position: 'fixed', inset: 0, zIndex: 250, pointerEvents: 'none',
        opacity: endOpacity,
        background: 'radial-gradient(circle at center, #f8f5ef 0%, #e0ddd6 100%)',
      }} />

      {/* End text — above the paper overlay */}
      <div
        onClick={() => { if (endOpacity > 0) targetDepth.current = 0 }}
        style={{
          position: 'fixed', top: '50%', left: 'clamp(24px, 5vw, 64px)',
          transform: 'translateY(-50%)',
          zIndex: 251, opacity: endOpacity,
      ...
      >
        <div style={{
          fontFamily: 'Instrument Serif, serif', fontSize: 'clamp(20px, 3.5vw, 56px)',
          color: '#1a1816', letterSpacing: '-0.04em', lineHeight: 1,
        }}>
          ↑
        </div>
        <div style={{ fontSize: 8, letterSpacing: 4, textTransform: 'uppercase', color: '#6a6660' }}>
          retour
        </div>
      </div>
