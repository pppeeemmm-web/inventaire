...
// Fiscal — expenses tracker + French BNC tax framework for artists.
...
export function Fiscal({ oeuvres, contacts = [] }: Props) {
  const { t } = useI18n()
  ...
  useEffect(() => {
    ...
    ;(sb.from('expense') as any)
      ...
      .then(({ data, error }: { data: Expense[] | null; error: { message: string } | null }) => {
        if (error) {
          console.error('[Fiscal] expense fetch error:', error.message)
          // Table may not exist yet — surface a clear message instead of empty list
          if (error.message?.includes('does not exist')) {
            console.warn('[Fiscal] Table `expense` missing in DB. Run fix_expense_and_document.sql.')
          }
        }
        setExpenses(data ?? [])
        setLoading(false)
  ...
  return (
    <div data-testid="atelier-fiscal-root" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
