...
- i18n: see **🌐 UI COPY** below (non-negotiable for anything user-facing). **New copy:** add FR+EN together in feature modules under [`lib/i18n/messages/`](lib/i18n/messages/) with `defineMessages()`; `DictKey` is derived from those modules plus the legacy registry. Legacy keys still live in [`lib/i18n/dictionary/keys.ts`](lib/i18n/dictionary/keys.ts) + [`fr.ts`](lib/i18n/dictionary/fr.ts) / [`en.ts`](lib/i18n/dictionary/en.ts) until touched.
- **ESLint:** `eslint-plugin-pem-i18n` (`file:eslint-rules`) — rule `pem-i18n/no-hardcoded-jsx-text` flags sentence-like JSX text, common copy attributes, and `alert`/`confirm`/`prompt`; legacy allow-off in [`.eslintrc.json`](.eslintrc.json) overrides (SalesTab, …). Run `npm run i18n:check` for missing translations, hardcoded-copy hotspots, and unused-key hints. **Public route metadata:** [`lib/i18n/route-metadata.ts`](lib/i18n/route-metadata.ts) `routeMetadata(route, lang)` + `dict` keys `seo_*`; do not hand-roll duplicate EN blocks on new `page.tsx`.
...
All user-visible copy → `useI18n().t(key, vars?)` + `lib/i18n/messages/*.messages.ts` for new keys (FR+EN side by side). Legacy copy may remain in `lib/i18n/dictionary/` until the owning surface is touched. Exceptions: DB text, proper nouns, immutable data. **Scrutiny:** before finish, run lint / `npm run i18n:check` and fix JSX string literals & `alert`/`confirm`/titles/placeholders — hardcoded FR/EN = fix.
