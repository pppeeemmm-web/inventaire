...
| `/atelier` | Main team portal (tabbed UI); `?tab=inventory` / `?tab=sales` / `?tab=pipeline` / `?tab=production` / `?tab=stock-take` / `?tab=notes` / `?tab=reports` / `?tab=exhibitions` redirect to segment routes; see §3 |
...
| [`lib/atelier/tab-routes.ts`](../lib/atelier/tab-routes.ts) | **`atelierTabHref`**, legacy `?tab=inventory|sales|pipeline|production|stock-take|notes|reports|exhibitions` → segment route redirects |
...
| Param | Behavior |
|-------|----------|
| `?tab=<Tab>` | Opens tab (`Tab` union in `TeamPortalClient`; persisted in `localStorage` as `pem_team_tab`). **`?tab=inventory`** / **`?tab=sales`** / **`?tab=pipeline`** / **`?tab=production`** / **`?tab=stock-take`** / **`?tab=notes`** / **`?tab=reports`** / **`?tab=exhibitions`** → server redirect to segment routes (other query params preserved) |
