...
> - **Work drawer stack (post-split):** [WorkDrawer.tsx](components/atelier/WorkDrawer.tsx) shell ≈300 lines; the heavy editor lives in [DrawerContent.tsx](components/atelier/work-drawer/DrawerContent.tsx) (~1.4k lines) — still where core identity, themes, images, and save/delete lifecycle concentrate (an older audit cited a ~2.2k-line monolithic drawer pre-extraction). Further split targets: typed panels already under [components/atelier/work-drawer/](components/atelier/work-drawer/).
...
**Accomplished (incremental UX pass):** narrow-viewport branches and safe-area aware patterns in the Atelier shell (`TeamPortalClient` / sidebar), **admin pending-review badge** on Audit entry, **i18n** additions for œuvres paging UI + drawer-adjacent copy + **WorkDrawer** new-contact modal / panel expand titles (`wf_drawer_*`, reused `contactEditor*` keys), **Playwright** smoke [`tests/atelier-oeuvres-paging-bar.spec.ts`](tests/atelier-oeuvres-paging-bar.spec.ts). Inventory list/grid uses **virtualization** ([`InventoryTab.tsx`](components/atelier/InventoryTab.tsx) + `@tanstack/react-virtual`). **Partial catalogue disclosure:** when loaded œuvres are fewer than `oeuvresPaging.totalCount`, the shell shows a persistent subset banner + header loaded/total, Overview caption, Inventory denominator hint, Production subtitle + pivot footnote, **Rapports** / **Thèmes** / **Portfolio** inline notes (dictionary keys `atelier_oeuvres_subset_banner`, `report_loaded_subset_note`, `ov_loaded_subset_caption`, etc.). This is not a claim that every ruthless UX bullet is closed.
...
> - The **single-load atelier page** is the spine; everything depends on it. It's also the scaling cliff. *(œuvres row cap removed in favor of counted first chunk + keyset “load more”; dead `exhibition` fetch removed; RSC ships empty reference sentinels + post-paint `fetchAtelierShellPostPaint` for contacts/addresses/lookups; not full per-tab lazy fetch.)*
...
**Read-path delta (Atelier):** Overview pipeline pulse still uses the browser Supabase client for `suivi_process` / `suivi_etape` (and other pulse widgets); **`suivi_reminder`** list data for overview + pipeline initial paint is server-sourced (`initialReminders`). Curation/compare addresses + shell reference tables hydrate post-mount via **`fetchAtelierShellPostPaint`** (replaces the older `fetchAtelierContactAddresses`-only description).
...
`CLAUDE.md` reflects: `lib/image-upload.ts`, `lib/work-pending-keys.ts`, `lib/types/database.ts` (`SuiviReminderListRow`, …), Phase B allow-list, calendar HKDF + `token_salt`, broadcast validation + RLS + **rate limit** (`lib/inventory-broadcast-rate-limit.ts`), OAuth opaque codes, audit **Contact** batch enrichment (not `getUserById`), dev login log masking, **chunked œuvres load + `fetchOeuvresKeysetPage`**, **partial-catalogue UX** (subset banner, header loaded/total, Overview caption, Reports/Themes/Portfolio notes, Inventory/Production subset copy, `PivotPanel` `footnote`), **`team-portal-types.ts`** for RSC→client props, `lib/data.ts` **no dictionary import** + bilingual status map, Inventory virtualization, reminders **server count + list + mark/insert** + `revalidateRemindersTag`, **`fetchAtelierShellPostPaint`** post-paint shell hydrate, **reads/bootstrap** pattern in `app/atelier/*-actions.ts`, Playwright / lint cmds, dev **404 on `/_next/static`** troubleshooting, **public SEO** (`lib/seo/site-url.ts`, `app/robots.ts`, `app/sitemap.ts`, `app/page.tsx` + `components/public/LandingPage.tsx`, **`NEXT_PUBLIC_SITE_URL` / `NEXT_PUBLIC_APP_URL`** for canonical `metadataBase`). **[`docs/ROADMAP.md`](docs/ROADMAP.md)** aggregates deferred / future work.

[`docs/SITE_MAP.md`](docs/SITE_MAP.md) lists `/robots.txt`, `/sitemap.xml`, the split home page, and Atelier partial-load **data-testid** hooks for QA.

**This file:** Keep **Accomplished / partial / not** in sync when closing ruthless-audit items; §2–§5 updated May 2026 for loader trim, reminders path, addresses typing, drawer groups extract, SITE_MAP loader note, **§2 public SEO / crawl hygiene** block, partial-catalogue disclosure, and **[`docs/ROADMAP.md`](docs/ROADMAP.md)**.
