...
- app/atelier/page.tsx — parallel `Promise.all` reference loads + **first œuvres chunk** (`limit` + exact total count); **not** in this bundle: `exhibition` rows (ExhibitionsTab self-fetches), `contact_addresses` (post-paint via `fetchAtelierContactAddresses` for curation/compare — **Carte** tab loads its own `contact_addresses` in `WorldMapTab`). Passes `initialReminders` + unread count from `reminders-actions`. More œuvres via `fetchOeuvresKeysetPage` in `TeamPortalClient` (no silent 5000-row cap).
