...
| `/atelier` | Main team portal (tabbed UI); `?tab=inventory` / `?tab=sales` / `?tab=pipeline` / `?tab=production` / `?tab=stock-take` redirect to segment routes; see §3 |
...
| [`lib/atelier/tab-routes.ts`](../lib/atelier/tab-routes.ts) | **`atelierTabHref`**, legacy `?tab=inventory|sales|pipeline|production|stock-take` → segment route redirects |
...
**Recent doc sync (2026-05-23):** Slice 3 segment routes `/atelier/inventory`, `/atelier/sales`, `/atelier/pipeline`, `/atelier/production`, `/atelier/stock-take`; shared `loadAtelierShellProps` + `tab-routes`; legacy `?tab=` redirects for migrated tabs.
