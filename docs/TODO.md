...
- [ ] **Four works whose last image is older than their cover** (2026-07-30) — #272, #1434, #2103, #2339. The app's rule is "highest `SeqNo` = cover", but on these the highest-SeqNo row predates the current cover by `DateAdded`: three carry a foreign `W_xxxx_01_` prefix inside their filename (imported from another work), and #2339 has `SeqNo` and filename seq swapped. [`cover_follows_newest_image.sql`](../supabase/sql/cover_follows_newest_image.sql) deliberately skips them. Needs eyes on the actual pictures to say which one should be the cover.
...
- **Data repairs** — 2026-07-30. Applied the 10 stranded staged photos (6 new `tblImage` rows on #2365–#2370; 4 skipped as duplicates by the new `sha256` guard — #2364 and #2356 already held their image, #2368 was staged twice byte-identical); staged shots 10 → 0. Merged duplicate `Support` "Carton" (8 → 1, 2 works repointed; orphan row 8 deletion still open). Elected a cover for the 24 works that had images but no `is_cover`, via the trigger so `txtImageNameLink` was never written directly — works without a cover 24 → 0, none left with two. SQL in [`support_carton_merge_and_cover_backfill.sql`](../supabase/sql/support_carton_merge_and_cover_backfill.sql). **That cover election picked the lowest `SeqNo` and so sent several covers back to the first photo of the work — corrected the same day, see the next entry.**
- **Cover follows the newest image; session slot repointed** — 2026-07-30. The inventory list reads `Oeuvres.txtImageNameLink` while the drawer opens on the last image, and the two disagreed on 7 works (#2357 showed its 07-13 shot in the list and its 07-18 shot in the drawer). Cause: `commitWorkImage()` wrote the `Oeuvres` column directly and never set `tblImage.is_cover`, so flag and column drifted; the earlier backfill then elected the lowest `SeqNo`. Now `commitWorkImage()` inserts with `is_cover = true` and `syncCover()` flags the highest-`SeqNo` row after a delete/reorder — the trigger owns the column in both paths. Backfill in [`cover_follows_newest_image.sql`](../supabase/sql/cover_follows_newest_image.sql): 14 rows re-flagged, every work now has exactly one cover and 0 column/flag disagreements (4 legacy works skipped, listed under Product — open). Separately, the 2026-07-29 journal day showed empty deleted #2371 instead of photographed #2362: the pre-`155f1c2` double-apply created a second `Oeuvre` from a stale snapshot and repointed the slot at it. Slot and session `oeuvre_id` repointed to #2362; `applyWorkSessionToOeuvre` now re-reads the slot before creating a work.
