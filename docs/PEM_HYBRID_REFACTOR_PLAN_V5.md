...
- Remove `off` overrides for tabs as they're migrated: ~~`ExhibitionsTab`~~ (`Exhibitions.tsx` migrated — override path updated), ~~`FiscalTab`~~ (`Fiscal.tsx` migrated — override path updated), ~~`BroadcastTab`~~ (`Broadcast.tsx` migrated — override path updated), ~~`InventoryTab`~~ (`Inventory.tsx` migrated — override path updated), `CurationPanel`, `PortfolioConfigShell`.
